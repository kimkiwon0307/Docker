...
<summary><kbd>📁 6장. 쿠버네티스의 기본 구조 </kbd></summary> 

## 6.1 Kubernetes의 개념

### 6.1.1 Kubernetes의 어원과 역사
* **정의**: 컨테이너를 자동으로 배포, 관리, 확장, 복구하는 오케스트레이션 플랫폼입니다.
* **Docker의 한계**: 애플리케이션(예: SpringBoot)이 중단될 경우 수동으로 재실행해야 하는 번거로움이 있어, 이를 자동 관리해 줄 도구가 필요합니다.
* **어원**: 그리스어로 '조타수', '선장', '배를 조종하는 사람'을 뜻합니다.
* **배경**: Docker(컨테이너)를 실은 배를 조종하는 사람이라는 의미에서 유래했습니다.
* **K8s**: K와 S 사이에 8개의 문자가 있어 줄여서 K8s라고 부릅니다.

### 6.1.2 Kubernetes의 역할
* **자동 배포 (Deployment)**
* **자동 복구 (Self-Healing)**
* **자동 확장 (Auto Scaling)**
* **로드 밸런싱**
